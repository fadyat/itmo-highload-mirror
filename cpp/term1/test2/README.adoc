...
* Используйте смоук-тесты (`src/test_task_*.cpp`) для локального тестирования
...
* часть тестов работает с class member operator new/delete — для подсчета вызванных методов
** реализации лежат в `custom_new_delete.cpp`, можно посмотреть на использование в тестах
