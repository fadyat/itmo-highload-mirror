[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/zzqt64-4)
= Семинар IV
:icons: font
:table-caption!:
...
** *NB:* в задаче `C from CPP` есть make
...
** забирание значения
* для быстродействия полезнее inlining (подстановка тела функции в место вызова, *компилятором*)
** inlining != *inline* (ключевое слово в плюсах — _вспомним, зачем оно?_)
...
* параметризуется *типом-функцией*
* умеет хранить и свободные функции и функторы и лямбды
...
Реализуйте перечисление `WeekDay` (стабильнее, чем `enum`), определяющее дни недели, и три функции для работы с ним
...
== #5 C from CPP [0.5 балла]

Требуется проиллюстрировать вызов библиотечного кода на C из C++
...
* `src/task_5/Makefile` (!): написать достаточный таргет `smoke_task_5`
