[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/SSop1Qjw)
...
** шаблонные от `KeyType`, `ValueType`, ...
** по ключу хранят значения
** notes: find, operator[], emplace
* (по остаточному принципу) godbolt про виртуальные методы

=== `map` vs `unordered_map`

|===
| Контейнер             | `std::map`                   | `std::unordered_map`

| Внутренняя структура    | Самобалансирующееся двоичное дерево поиска (обычно красно-чёрное) | Хеш-таблица
| Порядок элементов       | Упорядочены по ключу   | Неупорядочены
| Время доступа           | O(log n)                   | В среднем O(1), в худшем случае O(n)
| Потребление памяти        | Меньше (из-за структуры дерева) | Больше (из-за избыточности разрешения коллизий методом цепочек)
| Итерация                | Упорядоченно (сортировка по ключам) | Неупорядоченно
|===

=== `map` vs `unordered_map` - 2

Эта разность отражается в интерфейсах:
...
int main() {
    ...
    m[1] = "one";       // Inserts {1, "one"}
    std::cout << m[1];  // Outputs: one

    std::cout << m[2];  // Inserts {2, ""} (default-constructed std::string)
    std::cout << m.size(); // Outputs: 2
...
int main() {
    std::map<int, std::string> m;
    m.emplace(1, "one");  // Efficient insertion
    m.emplace(1, "uno");  // Does not insert; key 1 already exists
...
int main() {
    ...
    auto it = m.find(2);
    if (it != m.end()) {
        std::cout << it->second; // Outputs: two
    }
...
int main() {
    ...
    try {
        std::cout << m.at(2); // Outputs: two
        std::cout << m.at(3); // Throws std::out_of_range
...
=== m[a]=b vs m.emplace(a, b)

godbolt: https://godbolt.org/z/hdcnG911q

=== find vs contains + at/operator[]
...
----
    ...
    if (m.contains(2)) { // O(log N)
        std::cout << m.at(2); // O(log N)
    }
----
...
** заметить модификатор `noexcept`, убрать [**]
...
=== Покурим godbolt про вызовы виртуальных методов

* https://godbolt.org/z/vK8dsx4oM[link]
