[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/llhQl1Lt)
= Семинар III
:icons: font
:table-caption!:
...
https://godbolt.org/z/vqa7znd3K[godbolt-заготовка]
...
=== Саинтайзер для нахождения проблем
* Крутой инструмент
...
[.fragment]
* типичная семантика:
** список задается головным элементом `Node* head`
[.fragment]
...
* Корректно обращаться к элементам `v.data[i]` при `i < v.size` (проверка выхода не нужна)

ifdef::backend-revealjs[=== !]

* `pushBack` — добавляет элемент в конец, выделяет новую память (*2, начинает с 1) при `size == capacity_`
* `popBack` — "удаляет" последний элемент из массива (освобожденный `int` можно переиспользовать для новых значений)
...
* есть односвязный список из структур `Node`
