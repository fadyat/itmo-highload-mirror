[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/4kvI3ZCD)
...
Давайте поразбираемся с типами выражений в конкретном сниппете кода (в декларациях нас будут интересовать части "справа"):
...
std::string foo() {
    ...
    // составные части:
    // std::string_view{funcS} — prvalue, переходит в xvalue (т.к. дальше дергается метод)
    // 2 - prvalue
    // выражение целиком (самый внешний функтор):
    // .substr(...) — prvalue (возвращает самостоятельный std::string)
    std::string funcSubStr{std::string_view{funcS}.substr(2)};
    ...
    // c-string literals — lvalue (адресуемые: обычно в read-only данных)
    result += "bar";
...
добавьте в класс очереди поддержку итераторов (const и простой)
