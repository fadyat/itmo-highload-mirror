use crate::data::{
    BlockHash, HASH_LEN, TransactionHash, VerifiedBlock, VerifiedTransaction, WalletId,
};
...
impl BlockForest {
    ...
    pub fn next_max_hash(&self) -> BlockHash {
        ...
        if !next_index.is_multiple_of(EPOCH_SIZE as u64) {
    ...
    fn validate_block(&self, block: &VerifiedBlock) -> Result<()> {
        if let Some(prev) = self.find_block(&block.prev_hash) {
            ...
            if !block.index.is_multiple_of(EPOCH_SIZE as u64) && prev.max_hash != block.max_hash {
                bail!(
        ...
        if let Some(expected_max_hash) = self.compute_max_hash(block)
            && block.max_hash != expected_max_hash
        {
            bail!(
                "wrong max_hash: expected {:?}, got {:?}",
                expected_max_hash,
    ...
    fn compute_max_hash(&self, block: &VerifiedBlock) -> Option<BlockHash> {
        if !block.index.is_multiple_of(EPOCH_SIZE as u64) {
