...
pub fn parse_pkcs8_public(raw: &str) -> Result<RsaPublicKey> {
    ...
    RsaPublicKey::from_public_key_der(&der_bytes).context("failed to decode pkcs8 bytes")
}

pub fn parse_pkcs8_private(raw: &str) -> Result<RsaPrivateKey> {
    let der_bytes = decode_pkcs8_plaintext(raw)?;
    RsaPrivateKey::from_pkcs8_der(&der_bytes).context("failed to decode pkcs8 bytes")
}
