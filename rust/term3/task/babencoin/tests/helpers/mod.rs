...
use babencoin::{
    data::{Block, BlockHash, HASH_LEN, PeerMessage},
    node,
};
...
const TEST_ARTEFACTS_PATH: &str = "./test_artefacts";
...
impl Env {
    pub fn new(name: &'static str, mut config: node::Config) -> Self {
        // Create a deterministic seed from the name, should be replaced with Seeder::from
        //  after stabilization edition 2024 in rsa + rand infrastructure
        let mut hasher = DefaultHasher::new();
        name.hash(&mut hasher);
        let seed = hasher.finish();

        let port = ChaCha20Rng::seed_from_u64(seed).random_range(49152..65436)
        ...
        let dir = Path::new(TEST_ARTEFACTS_PATH).join(format!("{}_{}", name, dir_suffix));
...
pub fn generate_private_key() -> RsaPrivateKey {
    // generate_multi_prime_key(&mut rand::rng(), 32, 1024).unwrap()
    RsaPrivateKey::new(&mut rand::rng(), 1024).unwrap()
}
