...
impl OpCode {
    ...
    pub fn extract_address(self) -> Address {
        Address::new(self.0 & 0x0FFF)
    }

    pub fn extract_word(self, index: usize) -> Word {
        if index > 1 {
            panic!("Invalid word index: {}", index);
        }

        ((self.0 >> (8 * (1 - index))) & 0xFF) as u8
    }

    pub fn extract_nibble(self, index: usize) -> Nibble {
        if index > 3 {
            panic!("Invalid nibble index: {}", index);
        }

        let value = ((self.0 >> (4 * (3 - index))) & 0xF) as u8;
        Nibble::try_from(value).unwrap_or_else(|_| panic!("Invalid nibble value: {}", value))
    }
