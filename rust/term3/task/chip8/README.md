...
При выполнении данного задания вам не разрешается пользоваться стандартной библиотекой (`std::*`).  
В прочем, если она вам понадобилась, то вы скорее всего делаете что-то неправильно.  
Всё, что вам потребуется и чем вы уже могли привыкнуть пользоваться, на самом деле содержится в _основной_ библиотеке ([`core::*`](https://doc.rust-lang.org/core/)).
...
Запускается командой `cargo xtask run-test chip8-logo`.
Для запуска в отладочном режиме добавьте флаг `--debug`.
Для того, чтобы в отладочном режиме отображались какие-то значения, реализуйте методы трейта `DebugTarget` у `Interpreter`.
...
Запускается командой `cargo xtask run-test ibm-logo`.

В дополнение к прошлым командам, вам нужно реализовать команду `7xnn` - прибавление `nn` к регистру `vx`.

### 3.3. Тест `corax`

Запускается командой `cargo xtask run-test corax`.
...
Запускается командой `cargo xtask run-test flags`.
...
Запускается командой `cargo xtask run-test quirks`.
...
Запускается командой `cargo xtask run-test keypad`.
