...
class Task:
    ...
    def create(
        ...
        CONTAINERLAB_TASKS = ["static_routing", "static_routing_backup_link", "isis"]
    ...
    @classmethod
    def list(cls, repository_path: pathlib.Path):
        ...
        for path in repository_path.iterdir():
            ...
            tasks.append(Task(path.name, repository_path))

        return tasks

    def __init__(self, name, repository_path: pathlib.Path):
        self.name = name
    ...
    def check_regexp_ban(self, file):
        try:
            ...
            raise RuntimeError(
                "File {} contains non-unicode characters".format(file)
            ) from e
