...
class PingResult:
    ...
    ping_result_future: concurrent.futures.Future[Any]
...
class Pinger:
    ...
    def __init__(self, client: DockerClient, hosts: list[Host]) -> None:
        self.hosts: list[Host] = hosts
        self.docker_client: DockerClient = client
        self.executor: concurrent.futures.ThreadPoolExecutor = (
            concurrent.futures.ThreadPoolExecutor(max_workers=self.THREADPOOL_SIZE)
        )

    def ping_pairwise(self) -> bool:
        ...
        for ping_result in results:
            try:
                ret: PingResult = ping_result.ping_result_future.result(timeout=10)
        ...
        if success:
            ...
            logger.error("Some pings failed.")
