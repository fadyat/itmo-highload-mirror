...
def create_tar_from_bytes(filename: str, file_data: bytes) -> bytes:
    """
...
class DockerClient:
    ...
    DOCKER_CONNECTION_POOL_SIZE: int = 300

    def __init__(self, docker_socket: str, container_name_prefix: str) -> None:
    ...
    def execute_command_in_container(
        self, host_name: str, command: str | list[str], detach: bool = False
    ) -> str | None:
        """
        ...
        result = container.exec_run(
            command, stdout=True, stderr=True, tty=True, detach=detach
        )
        ...
        if result.exit_code is not None and result.exit_code != 0:
            ...
            raise RuntimeError(
                f"Failed to execute command in container {container_name}"
            )
        elif not detach:
            logger.debug(
        ...
        return str(result.output.decode()).strip() if not detach else None

    def put_file(
        self, host_name: str, directory: str, file_name: str, data: bytes
    ) -> None:
        """
