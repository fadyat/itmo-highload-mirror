...
\fancyhead[L]{ИТМО MHS, ФП 2025, Дз 3}
% ------------------------------------------------
...
\begin{document}
    ...
    \begin{proof}
        \[
            \begin{aligned}
                x & : X \\
                x\ \mathsf{K} & : T_1, \quad \mathsf{K} : \alpha \to \beta \to \alpha, \quad x : (\alpha \to \beta \to \alpha) \to T_1 \\
                x\ \mathsf{K}\ \mathsf{I} & : T_2, \quad \mathsf{I} : \gamma \to \gamma, \quad T_1 : (\gamma \to \gamma) \to T_2 \\
                x\ \mathsf{K}\ \mathsf{I}\ x & : X, \quad T_2 = X \\
                x & : (\alpha \to \beta \to \alpha) \to (\gamma \to \gamma) \to X \\
                \lambda x \ldotp x\ \mathsf{K}\ \mathsf{I}\ x & : (\alpha \to \beta \to \alpha) \to (\gamma \to \gamma) \to X \to X
            \end{aligned}
        \]
    \end{proof}
    ...
    \begin{proof}
        \[
            \begin{aligned}
                ...
                x & : X \\
                y & : Y \\
                g\ x & : T_1, \quad g : X \to T_1 \\
                g\ y & : T_2, \quad g : Y \to T_2, \quad X = Y, \quad T_1 = T_2 = T \\
                f\ g\ (g\ x)\ (g\ y) & : R, \quad f : (X \to T) \to T \to T \to R \\
                \lambda f~g~x~y \ldotp f\ g\ (g\ x)\ (g\ y) & : ((X \to T) \to T \to T \to R) \to (X \to T) \to X \to X \to R
            \end{aligned}
        \]
    \end{proof}
    ...
    \begin{proof}
        \[
            \begin{aligned}
                x & : X \\
                z & : Z \\
                z\ x & : T_1, \quad z : Z \to T_1, \quad x : Z \\
                y & : Y \\
                y\ z & : T_2, \quad y : Z \to T_1 \to T_2 \\
                y\ z\ (z\ x) & : T_3, \quad y\ z : T_1 \to T_3, \quad T_2 = T_1 \to T_3 \\
                x\ (y\ z\ (z\ x)) & : R, \quad x : T_3 \to R \\
                \lambda x~y~z \ldotp x~(y~z~(z~x)) & : (T_3 \to R) \to (Z \to T_1 \to T_3) \to (Z \to T_1) \to R
            \end{aligned}
        \]
    \end{proof}
    ...
    \begin{proof}
        \[
            \begin{aligned}
                ...
                y & : Y \\
                z & : Z \\
                y\ z & : T_1, \quad y : Z \to T_1 \\
                x\ z & : T_2, \quad x : Z \to T_2 \\
                x\ z\ (y\ z) & : T_3, \quad x\ z : T_1 \to T_3, \quad T_2 = T_1 \to T_3 \\
                x & : Z \to T_1 \to T_3 \\
                \lambda x~y~z \ldotp x\ z\ (y\ z)
                & : (Z \to T_1 \to T_3) \to (Z \to T_1) \to Z \to T_3
            \end{aligned}
        \]

        \begin{lstlisting}[language=Go,label={lst:s}]
        package main
    ...
    \begin{proof}
        \begin{gather*}
            \lambda x^\alpha~y^\alpha\ldotp x\\
            \lambda x^\alpha~y^\alpha\ldotp y
        \end{gather*}
    \end{proof}
    ...
    \begin{proof}
        \begin{gather*}
            \lambda x^\alpha~y^\alpha~z^\alpha\ldotp x\\
            \lambda x^\alpha~y^\alpha~z^\alpha\ldotp y\\
            \lambda x^\alpha~y^\alpha~z^\alpha\ldotp z
        \end{gather*}
    \end{proof}
    ...
    \begin{proof}
        Невозможно населить, так как $\alpha$ не является замкнутым термом.
    \end{proof}
    ...
    \begin{proof}
        \[
            \begin{aligned}
                M_0 &= \lambda x^\alpha~y^\beta~f^{\alpha \to \beta \to \gamma} \ldotp f~x~y \\
                M_1 &= \lambda x^\alpha~y^\beta~f^{\alpha \to \beta \to \gamma} \ldotp f~x~(f~x~y) \\
                M_2 &= \lambda x^\alpha~y^\beta~f^{\alpha \to \beta \to \gamma} \ldotp f~x~(f~x~(f~x~y)) \\
                &\;\;\vdots \\[1mm]
                M_n &= \lambda x^\alpha~y^\beta~f^{\alpha \to \beta \to \gamma} \ldotp f~x~(f~x~(\dots(f~x~y)\dots))
            \end{aligned}
        \]
    \end{proof}
    ...
    \begin{proof}
        \[
            \begin{aligned}
                M_0 &= \lambda f^{\alpha \to \alpha}~x^\alpha \ldotp x \\
                M_1 &= \lambda f^{\alpha \to \alpha}~x^\alpha \ldotp f~x \\
                M_2 &= \lambda f^{\alpha \to \alpha}~x^\alpha \ldotp f~(f~x) \\
                &\;\;\vdots \\[2mm]
                M_n &= \lambda f^{\alpha \to \alpha}~x^\alpha \ldotp f^n(x)
            \end{aligned}
        \]
    \end{proof}
    ...
    \begin{proof}
        \[
            \lambda f^{\alpha \to \beta \to \beta}~x^\beta \ldotp x
        \]
    \end{proof}
    ...
    \begin{proof}
        \begin{gather*}
            x:\alpha \;\vdash\; g\,x\,x : \beta \\
            \vdash \lambda x^\alpha.\, g\,x\,x : \alpha \to \beta \\
            f:(\alpha\to\beta)\to\alpha \;\vdash\; f(\lambda x^\alpha.\, g\,x\,x) : \alpha \\
            \vdash g\bigl(f(\lambda x^\alpha.\, g\,x\,x)\bigr)\bigl(f(\lambda x^\alpha.\, g\,x\,x)\bigr) : \beta \\
            M \;=\; g\bigl(f(\lambda x^\alpha.\, M)\bigr)\bigl(f(\lambda x^\alpha.\, M)\bigr) \\
            \vdash \lambda f^{(\alpha\to\beta)\to\alpha}\, g^{\alpha\to\alpha\to\beta}.\, M
            : ((\alpha\to\beta)\to\alpha)\to(\alpha\to\alpha\to\beta)\to\beta
        \end{gather*}
    \end{proof}
    ...
    \begin{proof}
        \begin{gather*}
            f : (((\alpha \to \beta) \to \alpha) \to \alpha) \to \beta \;\vdash\; f(\lambda x:(\alpha\to\beta)\to\alpha.\, x) : \beta \\
            M_0 = f(\lambda x:(\alpha\to\beta)\to\alpha.\, x) : \beta \\
            \lambda x:(\alpha\to\beta)\to\alpha.\, M_0 : ((\alpha\to\beta)\to\alpha) \to \alpha \\
            M_1 = f(\lambda x:(\alpha\to\beta)\to\alpha.\, M_0) : \beta \\
            \lambda x:(\alpha\to\beta)\to\alpha.\, M_1 : ((\alpha\to\beta)\to\alpha) \to \alpha \\
            \vdots \\
            M_{n+1} = f(\lambda x:(\alpha\to\beta)\to\alpha.\, M_n) : \beta
        \end{gather*}
    \end{proof}
    ...
    \begin{proof}
        \[\resizebox{0.8\hsize}{!}{
            \infer[intro\to]
            {\emptyset \vdash \lambda f~g~x.~f~x~(g~x) : (\alpha \to \beta \to \gamma) \to (\alpha \to \beta) \to \alpha \to \gamma}
            {
                \infer[intro\to]
                {\{f: \alpha \to \beta \to \gamma\} \vdash \lambda g~x.~f~x~(g~x) : (\alpha \to \beta) \to \alpha \to \gamma}
                {
                    \infer[intro\to]
                    {\{f : \alpha \to \beta \to \gamma, g : \alpha \to \beta\} \vdash \lambda x.~f~x~(g~x) : \alpha \to \gamma}
                    {
                        \infer[elim\to]{\{f : \alpha \to \beta \to \gamma, g : \alpha \to \beta, x:  \alpha\} \vdash f~x~(g~x) : \gamma}
                        {
                            \infer[elim\to]{\{f : \alpha \to \beta \to \gamma, g : \alpha \to \beta, x: \alpha\} \vdash f~x : \beta \to \gamma}
                            {
                                \infer[ctx]{\Gamma \vdash f : \alpha \to \beta \to \gamma}{} &
                                \infer[ctx]{\Gamma \vdash x : \alpha}{}
                            } &
                            \infer[elim\to]{\{f : \alpha \to \beta \to \gamma, g : \alpha \to \beta, x: \alpha\} \vdash g~x : \beta}
                            {
                                \infer[ctx]{\Gamma \vdash g : \alpha \to \beta}{} &
                                \infer[ctx]{\Gamma \vdash x : \alpha}{}
                            }
