...
\fancyhead[L]{ИТМО MHS, ФП 2025, Дз 2}
% ------------------------------------------------
...
\begin{document}
    ...
    \begin{proof}
        Перепишем уравнение в виде рекурсивного определения:
        \[
            F = \lambda x.\; x F.
        \]
        ...
        Тогда рекурсивное уравнение можно записать как
        \[
            F = \Phi F.
        \]

        По определению комбинатора неподвижной точки $Y$ получаем решение:
        \[
            F = Y \Phi,
        \]
        так как
        \[
            Y \Phi \eqbeta \Phi (Y \Phi) = \lambda x.\; x (Y \Phi) = \lambda x.\; x F.
        \]

        Проверка:
        \[
            F M = (\lambda x.\; x F) M \eqbeta M F
        \]
    \end{proof}
    ...
    \begin{proof}
        \begin{gather*}
            F = \lambda m n.\; F n (m F).\\
            \Phi = \lambda f m n.\; f n (m f),\\
            F = \Phi F.\\
            F = Y \Phi,\\
            Y \Phi \eqbeta \Phi (Y \Phi) = \lambda m n.\; (Y \Phi) n (m (Y \Phi)) = \lambda m n.\; F n (m F).\\
        \end{gather*}

        Проверка:
        \[
            F~M~N = (\lambda m n.\; F n (m F)) M N \eqbeta (\lambda n.\; F n (M F)) N \eqbeta F N (M F)
        \]
    \end{proof}
    ...
    \begin{proof}
        \begin{gather*}
            ...
            G = Y (\lambda g.\; B F g)\\
            F = A F G\\
            F = A F (Y \lambda g.\; B F g)\\
            F = (\lambda f.\; A f (B f (Y \lambda g.\; B f g))) F\\
            F = Y (\lambda f.\; A f (B f (Y \lambda g.\; B f g)))\\
            G = Y (\lambda g.\; B F g)\\
            G = Y (\lambda g.\; B (Y \lambda f.\; A f (B f (Y \lambda g.\; B f g))) g)\\
        \end{gather*}
    \end{proof}
    ...
    \begin{proof}
        Проблема в том, что числа Чёрча не хранят предыдущего значения, поэтому нельзя просто вычесть 1.
        Будем использовать \emph{пару} $(\text{текущее}, \text{предыдущее})$, которая обновляется на каждом шаге.
        После $n$ итераций второе значение пары будет равно $n-1$.

        \[
            \term{pred} = \lambda n f x.\; n\; (\lambda g h.\; h (g f))\; (\lambda u.\; x)\; (\lambda u.\; u)
        \]

        \textbf{Разбор формулы}
        \begin{itemize}
            \item $n$ --- число Чёрча, задаёт количество итераций.
            \item $(\lambda g h.\; h (g f))$ --- функция-шаг:
            \begin{itemize}
                \item $g$ хранит текущую функцию (накопленное применение $f$);
                \item $h$ хранит предыдущую функцию;
                \item результат $h(g f)$ обновляет пару: новое текущее = $g f$, новое предыдущее = $h(g f)$.
            \end{itemize}
            \item $(\lambda u.\; x)$ --- начальное значение, которое соответствует паре $(0,0)$.
            \item $(\lambda u.\; u)$ --- извлечение второго элемента пары после $n$ шагов; именно оно возвращает $n-1$.
        \end{itemize}

        \[
            \term{minus} = \lambda n m.\; m~\term{pred}~n
        \]
    \end{proof}
    ...
    \begin{proof}
        \begin{gather*}
            \term{isZero} = \lambda n.\; n (\lambda x.\; \term{false})\; \term{true}\\
            \term{not} = \lambda b.\; b\ \term{false}\ \term{true}\\
            \term{lt} = \lambda n m.\; \term{not}(\term{isZero}(\term{minus}~mn))\\
        \end{gather*}
    \end{proof}
    ...
    \begin{proof}
        \begin{gather*}
            \term{and} = \lambda p q.\; p q p\\
            \term{equal} = \lambda n m.\; \term{and}~(\term{isZero}(\term{minus}~n m))~(\term{isZero}(\term{minus}~m n))
        \end{gather*}
    \end{proof}
    ...
    \begin{proof}
        \begin{gather*}
            \term{pair} = \lambda a b f. f a b, \quad
            \term{fst} = \lambda p. p\,(\lambda a b. a), \quad
            \term{snd} = \lambda p. p\,(\lambda a b. b)\\
            \term{Fib} =
            \lambda n.\,
            \term{fst}\Big(
            R\,
            (\term{pair}~0~1)\,
            (\lambda p.\, \term{pair}(\term{snd}~p)~(\term{plus}(\term{fst}~p)(\term{snd}~p)))\,
            n
            \Big)\\
        \end{gather*}
    \end{proof}
    ...
    \begin{proof}
        \[
            \term{sum} = \lambda l.\; l~\term{plus}~\term{0}
        \]
    \end{proof}
    ...
    \begin{proof}
        \[
            \term{length} = \lambda l.\; l~(\lambda h t.\; \term{succ}~t)~\term{0}
        \]
    \end{proof}
    ...
    \begin{proof}
        \begin{gather*}
            \begin{aligned}
                \text{Аппликативная стратегия:} \quad
                \lambda y. z~(\lambda x. \mathsf{I}~(\mathsf{I}~\mathsf{I})~(\mathsf{I}~\mathsf{I}))~x~y
                &\;\to_\beta\; \lambda y. z~(\lambda x. \mathsf{I}~\mathsf{I}~(\mathsf{I}~\mathsf{I}))~x~y \\
                &\;\to_\beta\; \lambda y. z~(\lambda x. \mathsf{I}~\mathsf{I}~\mathsf{I})~x~y \\
                &\;\to_\beta\; \lambda y. z~(\lambda x. \mathsf{I}~\mathsf{I})~x~y \\
                &\;\to_\beta\; \lambda y. z~(\lambda x. \mathsf{I})~x~y
            \end{aligned}\\[1em]
            \begin{aligned}
                \text{Нормальная стратегия:} \quad
                \lambda y. z~(\lambda x. \mathsf{I}~(\mathsf{I}~\mathsf{I})~(\mathsf{I}~\mathsf{I}))~x~y
                &\;\to_\beta\; \lambda y. z~(\lambda x. (\mathsf{I}~\mathsf{I})~(\mathsf{I}~\mathsf{I}))~x~y \\
                &\;\to_\beta\; \lambda y. z~(\lambda x. \mathsf{I}~(\mathsf{I}~\mathsf{I}))~x~y \\
                &\;\to_\beta\; \lambda y. z~(\lambda x. \mathsf{I}~\mathsf{I})~x~y \\
    ...
    \begin{proof}
        ...
        Покажем, что такого терма не существует.

        По определению, терм находится в WHNF, если он имеет вид
        \[
            M = \lambda x_1 \dots \lambda x_n.\; R,
        \]
        где $R$ не является $\beta$-редексом, то есть не имеет вида $(\lambda y.P)\,Q$.

        Возможные формы $R$: переменная $y$, аппликация $y\,M_1 \dots M_k$, либо абстракция $\lambda z.P$. Если $R$ — абстракция, её можно включить в префикс внешних $\lambda$. После снятия всех внешних $\lambda$ остаётся терм, который не является редексом и не начинается с $\lambda$, следовательно, это либо переменная, либо аппликация с переменной в голове.

        Это в точности определение HNF. Следовательно, любой терм в WHNF автоматически находится в HNF, и терма, который в WHNF, но не в HNF, не существует.
    \end{proof}
    ...
    \begin{proof}
        Рассмотрим терм
        \[
            N = \lambda x.\; x\,((\lambda y. y)\,x).
        \]

        Терм находится в HNF, так как после удаления внешней
        \(\lambda\) получаем выражение вида
        \[
            x\,M,
        \]
        где в позиции головы стоит переменная \(x\).

        Однако он не находится в NF, так как внутри аргумента
        присутствует \(\beta\)-редекс
        \[
            (\lambda y. y)\,x.
        \]
        Следовательно, в терме существует \(\beta\)-редуцируемое подвыражение,
        и он не является нормальной формой.
    \end{proof}
